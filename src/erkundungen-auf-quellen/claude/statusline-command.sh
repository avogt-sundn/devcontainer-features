...
if [ -f /tmp/claude-current-file.txt ]; then
  current_file=$(cat /tmp/claude-current-file.txt)
  if [ -n "$current_file" ]; then
    short_file=$(echo "$current_file" | awk -F/ '{n=NF; if(n>=3) print $(n-2)"/"$(n-1)"/"$n; else print $0}')
    printf "  |  \033[0;90m%s\033[0m" "$short_file"
  fi
fi
